...
#LOCAL
datapoints <- read.csv("~/Downloads/radish_geo.csv", header=TRUE, sep=",")
...
#LOCAL
files <- list.files(path="~/Downloads/wc2.0_30s_bio/", pattern='tif', full.names=TRUE)
...
writeRaster(px,'~/Downloads/radish_presentday.tif',options=c('TFW=YES'))
...
test<-raster('~/Downloads/radish_presentday.tif')
plot(test)
